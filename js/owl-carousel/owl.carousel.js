...
;(function($, window, document, undefined) {
	...
	Owl.prototype.updateLocalContent = function () {
		...
		for (k = 0; k < this.num.oItems; k++) {
			...
			/* TODO custom code from shady */
			item.data('nav-img', item.children().data('nav-img'));
	...
	Owl.prototype.onDragEnd = function() {
		...
		/* TODO custom code from shady */
		if (this.settings.slides && this.settings.items < 2) {
			$.proxy(this.plugins.slides, 'refresh')(this.normalize(this.current(), true), this.plugins.slides);
		}

		/* TODO custom code from shady */
		if (this.settings.navThumb && this.settings.items < 2) {
			$.proxy(this.plugins.navigation.navThumbsAction, this)(this.normalize(this.current(), true), this.plugins.navigation, false);
		}
...
;(function($, window, document, undefined) {
	...
	var Slides = function (carousel) {
		this.core = carousel;
		this.core.options = $.extend({}, Slides.Defaults, this.core.options);
	...
	Slides.prototype.drawPaths = function () {
		var paths = [],
			template = '<li style="background-image: url(__URL__)"></li>';
		this.core.dom.$oItems.each(function (idx, item) {
			paths.push(template.replace(/__URL__/gi, $(item).children().data('bg-img')));
		});
		this.pathsArray = paths;
	}

	Slides.prototype.filterPaths = function (current) {
		return this.pathsArray[current];
	}
	...
	Slides.prototype.initialize = function () {
		...
		this.onAnimationEnd = function (item, callback) {
			var onEndFn = function () {
				callback.call();
			};
			item.one(this.animEndEventName, onEndFn);
		}

		this.isAnimating = false;
		this.items = this.core.dom.$oItems;
		this.index = this.core.normalize(this.core.current(), true);
	}

	Slides.prototype.draw = function () {
		this.template = $('<ul>', {
			'class': 'bg-post-slider ' + this.core.options.effect
		});

		this.template
			.prependTo($('.wrap'))
			.prepend(this.pathsArray.slice(0).reverse())
			.waitForImages({
				finished: $.proxy(function () {
	...
	Slides.prototype.refresh = function (current, self) {
		if (self.isAnimating) { return false; }
		self.isAnimating = true;

		if (current == -1) {
			current = this.core.num.oItems - 1;
		}
		if (current == this.core.num.oItems) {
			current = 0;
		}

		var html = this.filterPaths(current),
			itemChild = self.template.children('li:only-child');

		if (itemChild) {
			itemChild.addClass('old');
		}

		var onEndAnimFn = function () {
			var old = $('.old', self.template);
			if (old) {
				old.remove();
			}
			self.template.removeClass('effect-active');
			self.isAnimating = false;
		}

		self.template.append(html).addClass('effect-active');

		if (self.support) {
			self.onAnimationEnd( self.template.children('li'), onEndAnimFn );
		} else {
			onEndAnimFn.call();
		}
	...
	$.fn.owlCarousel.Constructor.Plugins.Slides = Slides;
			...
			'changed.owl.carousel': $.proxy(function(e) {
				if (e.property.name == 'items') {
					if (!this.initialized) {
						...
						/* TODO custom code from shady */
						if (this.core.settings.navThumb && this.core.settings.items < 2) {
							this.navThumbsInit(this.core.settings);
						}

						/* TODO custom code from shady */
//						if (this.core.settings.slides) {
//							$.proxy(this.core.plugins.slides, 'refresh')(this.core.normalize(this.core.current(), true), this.core.plugins.slides);
//						}
	...
	Navigation.Defaults = {
		...
		navThumb: false, /* TODO custom code from shady */
		navAltImgText: ['Previous thumb', 'Next thumb'], /* TODO custom code from shady */
		slideBy: 1,
		dotClass: 'owl-dot',
		dotsClass: 'owl-dots',
		dots: true,
...
//			.html(options.navText[1])
		...
		// override public methods of the carouselo
	 ...
	 * @protected
	 */
	Navigation.prototype.navThumbsInit = function (options) {
		this.animEndEventNames = {
			'WebkitAnimation' : 'webkitAnimationEnd',
			'OAnimation' : 'oAnimationEnd',
			'msAnimation' : 'MSAnimationEnd',
		...
		this.animEndEventName = this.animEndEventNames[Modernizr.prefixed('animation')];
		this.support = Modernizr.cssanimations;
		this.onAnimationEnd = function (elemPrev, elemNext, callback) {
			var onEndFn = function () {
				callback.call();
			};
			elemPrev.add(elemNext).one(this.animEndEventName, onEndFn);
		}

		this.controls.$previous.wrapInner('<div/>');
		this.controls.$next.wrapInner('<div/>');

		$('<span/>').addClass('arrow-content').prependTo(this.controls.$previous);
		$('<span/>').addClass('arrow-content').prependTo(this.controls.$next);

		this.controls.$previous.img = $('<img alt="' + options.navAltImgText[0] + '" src="">').addClass('work-image');
		this.controls.$next.img = $('<img alt="' + options.navAltImgText[1] + '" src="">').addClass('work-image');

		$.proxy(this.navThumbsAction, this.core)(this.core.normalize(this.core.current(), true), this, false);
	}

	/**
	 * TODO custom code from shady
	 * Nav Thumbs Action process
	 * @protected
	 */
	Navigation.prototype.navThumbsAction = function (index, self, direction) {

		if (index == this.num.oItems) { index = 0; }

		var	items = this.dom.$oItems,
			item = items.eq(index),
			itemData = item.data('owl-item'),
			indexPrev = itemData.index,
			indexNext = indexPrev + 1;

		var changeThumbs = function (prevItem, nextItem) {
			var itemPrevData = prevItem.data('nav-img'),
				itemNextData = nextItem.data('nav-img'),

				prevImg = self.controls.$previous.img.attr('src', itemPrevData),
				nextImg = self.controls.$next.img.attr('src', itemNextData);

			self.controls.$previous.children('div').append(prevImg);
			self.controls.$next.children('div').append(nextImg);
		}

		if (indexPrev === 0) {
			indexPrev = this.num.oItems - 1;
			$.proxy(changeThumbs, this)(items.eq(indexPrev), items.eq(indexNext));
		} else if (indexPrev < this.num.oItems - 1) {
			indexPrev -= 1;
			$.proxy(changeThumbs, this)(items.eq(indexPrev), items.eq(indexNext));
		} else if (indexPrev == this.num.oItems - 1) {
			indexPrev -= 1;
	...
	Navigation.prototype.next = function (speed) {
		...
		/* TODO custom code from shady */
		if ( this.core.settings.slides ) {
			$.proxy(this.core.plugins.slides, 'refresh')(this.core.normalize(this.core.current()), this.core.plugins.slides);
		}

		/* TODO custom code from shady */
		if ( this.core.settings.navThumb ) {
			$.proxy(this.navThumbsAction, this.core)(this.getPosition(true), this, 'next');
		}
	...
	Navigation.prototype.prev = function (speed) {
		...
		/* TODO custom code from shady */
		if ( this.core.settings.slides ) {
			$.proxy(this.core.plugins.slides, 'refresh')(this.getPosition(false), this.core.plugins.slides);
		}

		/* TODO custom code from shady */
		if ( this.core.settings.navThumb ) {
			$.proxy(this.navThumbsAction, this.core)(this.getPosition(false), this, 'prev');
		}
