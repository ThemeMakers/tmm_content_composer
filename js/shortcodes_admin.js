var TMM_APP_SHORTCODES = function() {
	var self = {
		...
		list_changer: function(shortcode) {
			...
			var shortcode_text = "";
			if (list_type == 0) {
				shortcode_text = begin_string + ' ' + list_item_styles + ']' + list_item_content + end_string;
			} else {
				shortcode_text = begin_string + ']' + list_item_content + end_string;
			}
			self.insert_html_in_buffer(shortcode_text);
		},
		album_changer: function(shortcode) {
			...
			//***
			var shortcode_text = begin_string +'post_per_page="'+jQuery('#post_per_page').val()+'"'+' gallery_slide_up="'+ jQuery('#gallery_slide_up').val() +'" gall_amount_icons="'+ jQuery('#gall_amount_icons').val() +'" show_filter="' + jQuery('#show_filter').val() + '" pagination="'+ jQuery('#album_pagination').val() +'"' +' layout="' + jQuery('#album_layout').val() + '"]' + list_item_content + end_string;
			self.insert_html_in_buffer(shortcode_text);
		},
