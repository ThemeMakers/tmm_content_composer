    ...
    public static function register()
    {
        ...
        while ($file = readdir($handler)) {
            if ($file !== "." && $file !== "..") {
                ...
                // Skip WooCommerce-related shortcodes if WooCommerce is not active
        ...
        if (!empty($shortcodes)) {
            foreach ($shortcodes as $value) {
                // Skip WooCommerce shortcodes if WooCommerce is not active
                if (strpos($value, 'woo') !== false && !class_exists('WooCommerce')) {
                    continue;
                }
        ...
        foreach ($shortcodes_keys as $shortcode_key) {
            // Skip WooCommerce-related shortcodes if WooCommerce is missing
            if (strpos($shortcode_key, 'woo') !== false && !class_exists('WooCommerce')) {
                continue;
            }
