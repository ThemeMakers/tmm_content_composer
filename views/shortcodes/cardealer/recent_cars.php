...
<?php if ( ! empty( $title ) ) : ?>
	...
	<div class="page-subheader">
		...
		<h2 class="section-title"><?php esc_html_e( $title, TMM_CC_TEXTDOMAIN ) ?></h2>
		...
		<?php if ( isset( $show_layout_switcher ) && $show_layout_switcher == true ) { ?>
			<div class="layout-switcher">
				<a class="layout-grid <?php echo( $cars_listing_layout_class == 'item-grid' ? 'active' : '' ) ?>"
				   ...
				   href="javascript:void(0);"><?php esc_html_e( "Grid View", TMM_CC_TEXTDOMAIN ) ?></a>
				<a class="layout-list <?php echo( $cars_listing_layout_class == 'item-list' ? 'active' : '' ) ?>"
				   data-css-class="item-list"
				   href="javascript:void(0);"><?php esc_html_e( "List View", TMM_CC_TEXTDOMAIN ) ?></a>
			</div><!--/ .layout-switcher-->
		<?php } ?>
...
<?php if ( $show_view_all_cars_link ):
	...
	?>
	<a href="<?php echo $searching_page; ?>"><?php esc_html_e( 'Show all cars', TMM_CC_TEXTDOMAIN ); ?> <i class="icon-angle-double-right"></i></a>
<?php endif; ?>
