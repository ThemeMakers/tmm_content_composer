...
?>
	<form method="post" class="contact-form" noValidate>
			...
			?>
				...
				?>
					<p class="input-block">
						...
						<textarea id="message_<?php echo esc_attr($unique_id) ?>" <?php echo esc_attr($input['is_required'] ? "  " : "") ?> name="<?php echo esc_attr($name) ?>"><?php echo esc_attr(!empty($_POST) ? $dont_fill_inputs ? "" : $_POST[$name]  : "") ?></textarea>
					</p>
				<?php
					break;
		...
		<div class="row">
			<?php if ($contact_form['has_capture']): ?>
				<div class="col-xs-6">
					<img id="captcha_image_<?php echo esc_attr($form_name) ?>" class="contact_form_capcha" src="<?php echo esc_url(get_template_directory_uri()); ?>/helper/capcha/image.php" width="200" height="70" alt="CAPTCHA image" />
					<input type="text" id="captcha_<?php echo esc_attr($form_name) ?>" name="captcha" class="verify" required>
				</div>
			<?php endif; ?>
