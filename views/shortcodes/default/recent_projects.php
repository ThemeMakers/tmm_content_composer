		...
		<?php foreach ($posts as $post) : ?>
			...
			<article class="<?php echo $effect_class; ?> <?php echo $column_class ?>">
				<a class="single-image link-icon" href="<?php echo get_permalink($post->ID) ?>">
						<img src="<?php echo TMM_Helper::get_post_featured_image($post->ID, '420*300'); ?>" alt="<?php echo $post->post_title ?>" />
				</a>
			</article>
...
<?php if ($show_button): ?>
	<a class="button small default" href="<?php echo get_post_type_archive_link(TMM_Portfolio::$slug) ?>"><?php _e('See all projects', 'tmm_shortcodes'); ?></a>
<?php endif; ?>
